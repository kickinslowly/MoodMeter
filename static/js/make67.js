(function(){
      ...
      if (data && data.ok){
        ...
        if (data.me){
          ...
          if (lastKnownRankKey !== null && newRankKey && newRankKey !== lastKnownRankKey &&
            ...
            // Play random rank-up sound
            try { playSfx(randomChoice(LEVELUP_SOUNDS)); } catch(_){ }
  ...
  // --- Color Theme Menu UI ---
  function _prefThemeKey(){ return 'm67_pref_theme:' + (myUid || 'anon'); }
  ...
  function showMrARain(payload){
    ...
    try { playSfx('snd_rain'); } catch(_){}
    try { if (navigator.vibrate) navigator.vibrate([80, 40, 120]); } catch(_){}
    // ACK the display so the server stops resending the payload. Fire-and-forget.
    ...
    // Refresh HUD so boost indicator appears immediately
  ...
  applyEmpowerment();
  // Initialize color theme (restore saved preference)
  initColorTheme();
  ...
  (function initSwipeSolve(){
    ...
    const _prefSwipeKey = 'm67_pref_swipe:' + (myUid || 'anon');
